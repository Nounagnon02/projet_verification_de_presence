...
class QrCodeController extends Controller
{
    ...
    public function generateSecure(Request $request)
    {
        $userGroup = Auth::user()->group;
        
        // Utiliser un code unique pour plus de sécurité
        $uniqueCode = QrCode::generateUniqueCode($userGroup);
        ...
        return view('qr.generate-secure', compact('qrCode', 'qrImage'));
    }
    ...
    public function refresh()
    {
        ...
        // Trouver le QR code actuel pour ce groupe
        $qrCode = QrCode::where('group', $userGroup)
                        ->where('event_date', today())
                        ->where('created_by', Auth::id())
                        ->first();
        
        if ($qrCode) {
            // Utiliser le code actuel (qui peut avoir été régénéré)
            $currentCode = $qrCode->code;
        } else {
            // Fallback vers le code basé sur le temps
    ...
    public function markPresence(Request $request, $code)
    {
        ...
        // RÉGÉNÉRER LE QR CODE APRÈS SCAN RÉUSSI
        $qrCode->regenerateAfterScan();
