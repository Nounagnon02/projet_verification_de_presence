        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 p-4 bg-accent-tint border border-accent/20 rounded-lg">
                <div class="flex items-start">
                    ...
                    <div class="text-sm text-accent">
                        <p class="font-semibold">{{ __('Ce qui fonctionne aujourd\'hui') }}</p>
                        <p class="mt-1">{{ __('« Vérifier maintenant » affiche les membres qui n\'ont pas encore pointé, à relancer vous-même (téléphone). L\'envoi automatique de SMS et de rappels n\'est pas encore actif : les réglages ci-dessous sont enregistrés en prévision, mais ne déclenchent aucun envoi pour l\'instant.') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-card dark:bg-gray-800 rounded-lg shadow-sm p-6">
                <form method="POST" action="{{ route('alerts.update', $group) }}">
                    ...
                    <div class="mb-6 p-4 bg-paper dark:bg-gray-700 rounded-lg">
                        <label class="flex items-center cursor-pointer">
                            ...
                            <span class="ml-3 text-lg font-semibold text-ink dark:text-white">
                                {{ __('Activer les alertes et rappels') }}
                            </span>
                        </label>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="space-y-4">
                            <label class="flex items-center">
                                ...
                                <span class="ml-2 text-ink-soft dark:text-gray-300">{{ __('Activer les alertes d\'absence') }}</span>
                            </label>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-ink-soft dark:text-gray-300 mb-1">
                                        {{ __('Heure de début d\'événement') }}
                                    </label>
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-ink-faint dark:text-gray-400 mt-1">
                                        {{ __('Envoyer une alerte X minutes après le début') }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-ink-soft dark:text-gray-300 mb-1">
                                    {{ __('Message d\'alerte personnalisé') }}
                                </label>
                                <textarea name="alert_message_template" rows="3"
                                          class="w-full rounded-lg border-line-strong dark:border-gray-700 dark:bg-gray-900"
                                          placeholder="{{ __('Bonjour {name}, vous n\'êtes pas encore enregistré pour l\'événement du {date}...') }}"
                                >{{ $settings->alert_message_template }}</textarea>
                                <p class="text-xs text-ink-faint dark:text-gray-400 mt-1">
                                    {{ __('Variables disponibles : {name}, {date}, {event}') }}
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            <label class="flex items-center">
                                <input type="checkbox" name="reminders_enabled" {{ $settings->reminders_enabled ? 'checked' : '' }}
                                       class="rounded border-line-strong dark:border-gray-600 text-accent">
                                <span class="ml-2 text-ink-soft dark:text-gray-300">{{ __('Activer les rappels avant événements') }}</span>
                            </label>

                            <div>
                                <label class="block text-sm font-medium text-ink-soft dark:text-gray-300 mb-1">
                                    {{ __('Envoyer rappel (heures avant)') }}
                                </label>
                    ...
                    <div class="mb-6">
                        <h3 class="text-lg font-semibold text-ink dark:text-white mb-4">{{ __('Canaux de notification') }}</h3>
                        <div class="space-y-3">
                            <label class="flex items-center opacity-50">
                                ...
                                <span class="ml-2 text-ink-soft dark:text-gray-300">{{ __('SMS (bientôt disponible)') }}</span>
                            </label>
                            <label class="flex items-center opacity-50">
                                <input type="checkbox" name="email_enabled" disabled
                                       class="rounded border-line-strong dark:border-gray-600">
                                <span class="ml-2 text-ink-soft dark:text-gray-300">{{ __('Email (bientôt disponible)') }}</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="mb-6 p-4 bg-paper dark:bg-gray-700 rounded-lg">
                        <h3 class="text-sm font-semibold text-ink dark:text-white mb-3">
                            {{ __('Contact administrateur (pour recevoir les résumés)') }}
                        </h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs text-ink-soft dark:text-gray-400 mb-1">{{ __('Téléphone') }}</label>
                                <input type="text" name="admin_phone" value="{{ $settings->admin_phone }}"
                                       class="w-full rounded-lg border-line-strong dark:border-gray-700 dark:bg-gray-900">
                            </div>
                            <div>
                                <label class="block text-xs text-ink-soft dark:text-gray-400 mb-1">{{ __('Email') }}</label>
