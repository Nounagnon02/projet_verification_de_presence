<x-app-layout>
    <x-slot name="header">
        <h1 class="text-2xl md:text-[28px] text-ink">{{ __('Analyses avancées') }}</h1>
        <p class="text-ink-soft text-base mt-1">{{ __('Tendances, classement des membres et comparaison de périodes.') }}</p>
    </x-slot>
    ...
    <div class="bg-card rounded-xl border border-line mb-8">
        <div class="p-6">
            <form method="GET" class="flex items-center gap-4">
                <x-input-label for="periode" class="mb-0 flex-shrink-0">{{ __('Période') }}</x-input-label>
                <select id="periode" name="periode" onchange="this.form.submit()"
                    ...
                    <option value="7" {{ $periode == 7 ? 'selected' : '' }}>{{ __('7 derniers jours') }}</option>
                    <option value="30" {{ $periode == 30 ? 'selected' : '' }}>{{ __('30 derniers jours') }}</option>
                    <option value="90" {{ $periode == 90 ? 'selected' : '' }}>{{ __('3 derniers mois') }}</option>
                    <option value="365" {{ $periode == 365 ? 'selected' : '' }}>{{ __('12 derniers mois') }}</option>
                </select>
            </form>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
        ...
        <div class="p-6 rounded-xl border border-line bg-card">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="font-display font-semibold text-ink">{{ __('Présences (période)') }}</h3>
            </div>
        ...
        <div class="p-6 rounded-xl border border-line bg-card">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="font-display font-semibold text-ink">{{ __('Moyenne / jour') }}</h3>
            </div>
