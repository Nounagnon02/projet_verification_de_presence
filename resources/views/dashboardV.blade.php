<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            @forelse($groups as $group)
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg transition-colors duration-300 mb-6">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">{{ $group->name }} - {{ now()->format('d/m/Y') }}</h3>
                        ...
                        @if($group->members->count() > 0)
                            <form method="POST" action="{{ route('verif', $group) }}">
                                ...
                                <div class="flex justify-between items-center">
                                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ __('messages.members_total', ['count' => $group->members->count()]) }}</p>
            ...
            @empty
                <p class="text-gray-500 dark:text-gray-400 text-center py-8">Vous ne dirigez aucun groupe pour le moment.</p>
            @endforelse
        </div>
    </div>
</x-app-layout>
