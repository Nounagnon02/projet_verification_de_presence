<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('dashboard') }}" aria-label="Retour au tableau de bord" class="w-10 h-10 rounded-lg flex items-center justify-center text-ink hover:bg-paper2 transition-colors flex-shrink-0">
    ...
    <div class="max-w-md mx-auto rounded-xl overflow-hidden border border-line">
        ...
        <div class="bg-[#332C25] flex flex-col items-center justify-center gap-6 p-8">
            ...
            <p class="text-[#F3ECDF] text-base text-center max-w-xs leading-relaxed">
                Placez le QR du membre dans le cadre.
            </p>
        </div>

        <!-- État du scan -->
        <div id="scan-waiting" class="px-5 py-6 bg-paper2" style="display:flex; align-items:center; gap:14px;">
            <span class="text-base text-ink-soft">En attente d'un scan…</span>
        </div>
        ...
        <div class="px-5 py-6 bg-card border-t border-line">
            <form method="POST" action="{{ route('sessions.close', $session) }}" onsubmit="return confirm('Fermer la session ?')">
                @csrf
                <x-secondary-button type="submit" class="w-full">Fermer la session</x-secondary-button>
            </form>
        </div>
    </div>
