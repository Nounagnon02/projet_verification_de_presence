<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('success'))
                <div class="bg-confirm-tint border border-confirm/30 text-confirm px-4 py-3 rounded-lg mb-4 mx-2 md:mx-0">
                    {{ session('success') }}
                </div>
            @endif

            @if(session('error'))
                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg mb-4 mx-2 md:mx-0">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mx-2 md:mx-0">
                ...
                <div class="lg:col-span-2">
                    <div class="bg-card overflow-hidden shadow-sm rounded-lg" x-data="{ showAddForm: {{ $errors->any() ? 'true' : 'false' }} }">
                        <div class="p-6 text-ink">
                            <div class="flex justify-between items-center mb-6">
                                <div>
                                    <h3 class="text-lg font-medium">{{ __('Gestion des membres') }}</h3>
                                    <p class="text-sm text-ink-soft">{{ trans_choice('Total : :count membre|Total : :count membres', $membres->total()) }}</p>
                                </div>
                                ...
                                <div class="overflow-x-auto">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        <thead class="bg-paper">
                                            <tr>
                                                <th class="px-4 py-3 text-left text-xs font-medium text-ink-faint uppercase tracking-wider">
                                                    {{ __('Membre') }}
                                                </th>
                                                <th class="px-4 py-3 text-left text-xs font-medium text-ink-faint uppercase tracking-wider">
                                                    {{ __('Téléphone') }}
                                                ...
                                                <tr class="hover:bg-paper transition-colors">
                                                    <td class="px-4 py-4 whitespace-nowrap">
                                                        <div class="flex items-center">
                                                            <div class="w-10 h-10 bg-{{ $memberScore['color'] }}-100 rounded-full flex items-center justify-center mr-3">
                                                                <span class="text-{{ $memberScore['color'] }}-600 font-semibold text-sm">
                                                                    {{ strtoupper(substr($membre->name, 0, 2)) }}
                                                                </span>
                                                            </div>
                                                            <div>
                                                                <div class="text-sm font-medium text-ink">{{ $membre->name }}</div>
                                                    ...
                                                    <td class="px-4 py-4 whitespace-nowrap">
                                                        <div class="flex flex-col items-center">
                                                            ...
                                                            <div class="flex items-center mb-1">
                                                                @for($i = 1; $i <= 5; $i++)
                                                                    <svg class="w-4 h-4 {{ $i <= $memberScore['stars'] ? 'text-yellow-400' : 'text-gray-300 ' }}" fill="currentColor" viewBox="0 0 20 20">
                                                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                                    </svg>
                                                                @endfor
                                                            </div>
                                                            <!-- Barre de progression -->
                                                            <div class="w-full bg-paper2 rounded-full h-2">
                                                            ...
                                                            <span class="text-xs font-semibold text-{{ $memberScore['color'] }}-600 mt-1">
                                                                {{ $memberScore['score'] }}%
                                                            </span>
                                                        </div>
                                                    </td>
                                                    <td class="px-4 py-4 whitespace-nowrap text-center">
                                                        ...
                                                        <a href="{{ route('membres.edit', $membre->id) }}"
                                                           class="text-accent hover:text-indigo-900 mr-3"
                                                           title="{{ __('Modifier') }}">
                                                            <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @else
                                <div class="text-center py-8">
                                    ...
                                    <p class="text-ink-faint">{{ __('Aucun membre enregistré pour le moment.') }}</p>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-card overflow-hidden shadow-sm rounded-lg">
                        <div class="p-6">
                            ...
                            @if(isset($ranking) && count($ranking) > 0)
                                <div class="space-y-3">
                                    @foreach($ranking as $index => $rank)
                                        <div class="flex items-center p-3 rounded-lg {{ $index === 0 ? 'bg-yellow-50 border border-yellow-200 ' : 'bg-paper ' }}">
                                            <div class="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full {{ $index === 0 ? 'bg-yellow-400 text-white' : ($index === 1 ? 'bg-gray-400 text-white' : ($index === 2 ? 'bg-orange-400 text-white' : 'bg-paper2 text-ink-soft ')) }} font-bold text-sm">
                                                {{ $index + 1 }}
                                            </div>
                                            ...
                                            <div class="flex-shrink-0 text-right">
                                                <span class="text-lg font-bold {{ $index === 0 ? 'text-yellow-600 ' : 'text-ink ' }}">
                                                    {{ $rank['score'] }}%
                                                </span>
                                            </div>
                                        </div>
                            ...
                            @else
                                <div class="text-center py-8 text-ink-faint">
                            ...
                            <div class="mt-6 pt-4 border-t border-line">
                                <p class="text-xs font-medium text-ink-faint mb-2">{{ __('Niveaux de régularité :') }}</p>
                                <div class="grid grid-cols-2 gap-2 text-xs">
                                    <div class="flex items-center">
                                        <div class="w-3 h-3 bg-green-500 rounded-full mr-2"></div>
                                        <span class="text-ink-soft">{{ __('Excellent (≥90%)') }}</span>
                                    </div>
                                    <div class="flex items-center">
                                        <div class="w-3 h-3 bg-blue-500 rounded-full mr-2"></div>
                                        <span class="text-ink-soft">{{ __('Bon (≥70%)') }}</span>
                                    </div>
                                    <div class="flex items-center">
                                        <div class="w-3 h-3 bg-yellow-500 rounded-full mr-2"></div>
                                        <span class="text-ink-soft">{{ __('Moyen (≥50%)') }}</span>
                                    </div>
                                    <div class="flex items-center">
                                        <div class="w-3 h-3 bg-orange-500 rounded-full mr-2"></div>
                                        <span class="text-ink-soft">{{ __('Faible (≥30%)') }}</span>
                                    </div>
                                    <div class="flex items-center">
                                        <div class="w-3 h-3 bg-red-500 rounded-full mr-2"></div>
                                        <span class="text-ink-soft">{{ __('Critique (<30%)') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
